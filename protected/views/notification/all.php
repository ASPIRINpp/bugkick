...
Yii::app()->clientScript->registerCss('whiteBackground', '#content {background-color:#fff;}');
Yii::app()->clientScript->registerScript('reloader', 'setTimeout(\'window.location.reload()\', 30000)');
?>
...
<script id="notificationFn" type="text/TemplateFn">
    ...
    ?>
    <div class="infoBox">            
    [<b>{{=it.project_name}}</b>] {{=it.name}}: {{=it.content}}
