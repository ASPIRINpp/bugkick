    ...
    protected function makeUrlNameUnique() {
        ...
        if (!$this->isNewRecord) {
            $isUrlNameExists = $this->exists(
                    'url_name=:url_name AND project_id!=:project_id', array(':url_name' => $this->url_name, ':project_id' => $this->project_id)
            );
            if ($isUrlNameExists)
            ...
            $isUrlNameExists = $this->exists(
                    'url_name=:url_name', array(':url_name' => $this->url_name)
            );
            if ($isUrlNameExists) {
...
SQL
                ...
                case 'local':
                    if (is_file('images/project_logo/' . $this->logo)) {
                        return ImageHelper::thumb(
                                        $width, $height, 'images/project_logo/' . $this->logo, $quality
                        );
                    }
                    break;
            }
    ...
    public function isCompanyAccessAllowed() {
        ...
        return UserByCompany::model()->exists(
                        'company_id=:company_id AND user_id=:user_id', array(
    ...
    public static function isProjectAccessAllowed($projectID = '', $userID = '') {
        ...
        return UserByProject::model()->exists(
                        'project_id=:project_id AND user_id=:user_id', array(
    ...
    public static function getProjectSettings() {
        ...
        return empty($project) || empty($project->project_settings) ? null : $project->project_settings;
    }
    ...
    public function addUser(User $user, $isAdmin = 1) {
        ...
        return $userByProject->save() && $this->company->addUser($user, $isAdmin) !== null ? $userByProject : null;
    }
    ...
    public static function getInvitePeopleListData($projectID = '') {
        ...
        if (!empty($project)) {
            ...
            if (is_array($projectUsers) && is_array($companyUsers)) {
                $users = array_diff(
                        CHtml::listData($companyUsers, 'user_id', 'name'), CHtml::listData($projectUsers, 'user_id', 'name'));
            }
        }
        return $users;
    }
