    ...
    public function run() {
        if (empty($this->user) || empty($this->company)) {
            Yii::app()->user->setFlash(
                    'error', Yii::t('main', 'You have to be logged-in within some company.')
            );
        ...
        if (!empty($paymentFormAttributes)) {
            ...
            if ($this->viewData['model']->validate()) {
                ...
                $paymentData = array_merge(
                        $paymentFormAttributes, $this->getStripePaymentData($paymentFormAttributes['interval']) // the keys of this array will override same keys from $paymentFormAttributes
                );
            }
        }
        try {
