...
class EditAction extends Action {
    ...
    public function run() {
        ...
        $project = $isNewProject ? new Project() : Project::model()->findByPk($project_id);
        if (!empty($project))
                ...
                $this->controller->respond(
                        array(
                            'html' => $this->controller->renderPartial(
                                    '//payment/need-upgrade', array('message' => $message), true
                            ),
                            'success' => false,
                            'limit' => 1
                        )
        ...
        foreach ($companies as $company)
            $this->viewData['companies'][$company->company_id] = $company->company_name;
