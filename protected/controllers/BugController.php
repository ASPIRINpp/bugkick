...
class BugController extends Controller {
    ...
    public function actionView($id) {
        ...
        Yii::app()->clientScript->registerScript(
                __CLASS__ . '#SyntaxHighlighter', <<<JS
...
//            Yii::app()->baseUrl . '/js/bugkick/task/common.js'
        ...
        $bug = $this->bug = Bug::model()->resetScope()
                ...
                ->find(
                'number=:number AND project_id=:project_id', array(':number' => $id, ':project_id' => $currentProject->project_id)
        );
        if (!empty($bug)) {
            Yii::app()->clientScript->registerScript(
                    __CLASS__ . '#bugkick.viewData', <<<JS
...
JS
    ...
    protected function initBugForm() {
        ...
        if ($this->bug->title != $this->bug->description) {
            $this->bug->description = str_replace('&#133;', '', $this->bug->title)
            ...
            if ($form->validate()) {
                ...
                if (empty($project)) {
                    Yii::app()->user->setFlash(
                            'error', 'Please choose the project first.'
                    );
                ...
                $openStatus = Status::model()->find(
                        'label=:label AND company_id=:company_id', array(':label' => 'New', 'company_id' => Company::current())
                );
                if (!empty($openStatus)) {
    ...
    public function actionUpdate($id) {
        ...
        $this->updateBug($bugModel, 'bug-update-form', array($this, 'onBugUpdateSuccess'));
        Yii::app()->end();
    }
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            try {
                ...
                $prevBug = $this->getProjectBugByNumber(
                        $bug->prev_number, $bug->project_id
                );
                $nextBug = $this->getProjectBugByNumber(
                        $bug->next_number, $bug->project_id
                );
    ...
    public function actionIndex() {
        ...
        if (isset($filterID) && $filterID > 0) {
            $userFilter = Filter::model()->find('user_id=:user_id AND filter_id=:filter_id', array('user_id' => User::getId(), 'filter_id' => $filterID));
            if ($userFilter) {
...
//            : '';
        ...
        $criteria->with = array(
            ...
            'label' => !isset($labelsSQL) ? array() : array('condition' => $labelsSQL . ' ' . $createdWithAPI . ' )', 'together' => true),
            'user' => !isset($usersSQL) ? array() : array('condition' => $usersSQL, 'together' => true),
            'user',
            'company',
            'commentCount',
        );
        ...
        $this->clientScript->registerScript(
                __CLASS__ . '#disable_selection', '$("body").attr("unselectable", "on").addClass("unselectable");', CClientScript::POS_READY
        );
    ...
    public function loadModel($id) {
        ...
        $model = Bug::model()->resetScope()->find(
                'number=:number AND project_id=:project_id', array(
            ':number' => (int) $id,
            ':project_id' => $project->project_id
                )
        );
    ...
    public function actionDndPrioritySort() {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            InstantMessage::instance()->sendToAll(
                    Project::getCurrent()->project_id, MessageType::TICKETS_ORDER_CHANGED
            );
            Yii::app()->end();
        } else
            throw new CHttpException(400, 'Invalid request.');
